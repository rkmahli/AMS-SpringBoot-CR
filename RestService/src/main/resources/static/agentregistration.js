$(function () {
    ...
    $("#idType").on("change", function () {
        ...
        else if ($('#idType').val() == 'Driving Licence') {
            $('#apidfield').empty();
            $(drive).appendTo('#apidfield');
        }
        else if ($('#idType').val() == 'Voter Id') {
            $('#apidfield').empty();
...
function cities() {
    ...
    for (i = 0; i < json[key].length; i++) {
        $("#city").append('<option value="' + json[key][i] + '">' + json[key][i] + '</option>');
    }
}
